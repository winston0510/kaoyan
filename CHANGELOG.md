...
### 新增
- **4 枚苹果风 PNG 应用图标**（GDI+ 矢量绘制，紫色垂直渐变底 #8165FF→#382DAA + 白色翻开书本图形 + 紫色书脊线 + 右下深紫圆底白色对勾徽章）：
...
### 部署
- **线上部署完成（2026-08-31）**：`dist-deploy.zip`（70 文件，正斜杠路径，1,090,818 B）经 Netlify Deploy API 发布到 `llss.netlify.app`，deploy `6a955e8e925ac48455feba70` 状态 ready（published 2026-08-31T10:59:27Z，error 空）。
- **线上验证全绿**：`sw.js` 返回 `kaoyan-v4.2.1`、首页引用 `assets/index-CzUDMMwG.js`、四枚 PNG 图标全部 200、`manifest.webmanifest` 含全部 3 条 PNG 图标条目。
- **使用方式**：iPhone Safari 打开 `llss.netlify.app` → 「分享」→ 「添加到主屏幕」，即可以全屏独立应用打开。
- **GitHub 仓库同步（2026-08-31）**：`public/manifest.webmanifest` + `public/sw.js`（commit `c353b0e`）、`index.html`（commit `67a88f1`）、`src/constants.ts`（commit `a907007`）经 MCP `push_files` 推送；遗留临时文件已删除（commit `aa66eff` / `88024a5` / `075ad50`）。4 枚 PNG 为二进制，MCP 通道仅支持文本，暂以线上部署与本地为准。

...
- **GitHub 同步**：sw.js+constants.ts（`77cf1d6`）、integrate_all.py（`184e633`）、gen_real_circuit.js（`33e0602`）、CHANGELOG 精要条目（`fd2e70c`→本提交，期间多次修复节丢失并将旧版本节压缩精要）；seed_real_circuit.sql（41.1 KB）超 MCP 单次推送上限，不入 GitHub，权威记录以本地为准。

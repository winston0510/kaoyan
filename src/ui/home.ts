...
import { activeDayKeys, dayCount, daysForDisplay, localTodayDays } from '../stats';
...
export function renderHome(): void {
  const days = daysForDisplay(localTodayDays());
  const today = dayCount(days, todayKey());
  ...
  if (el4) el4.textContent = String(calcStreak(activeDayKeys(days)));

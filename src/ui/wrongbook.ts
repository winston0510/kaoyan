...
export function renderWrongBook(): void {
  ...
  const listHtml = filtered.map(q => {
    ...
    if (q.options && q.options.length) {
      const rightLetters = answerLetters(q.answer || '');
      const pickedLetters = answerLetters(q.userAnswer || '');
      optHtml = q.options.map((opt, i) => {
        const letter = String.fromCharCode(65 + i);
        const cls = rightLetters.includes(letter) ? 'correct-opt' : (pickedLetters.includes(letter) ? 'wrong-opt' : '');
    ...
    return `<div class="wrong-card">
      ...
      <div class="wc-answer" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'':'none'">
        <span>你的答案：<b style="color:var(--danger)">${esc(q.userAnswer)}</b></span><span style="color:var(--primary)">查看答案</span>
      </div>
